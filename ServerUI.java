...
public class ServerUI extends JFrame{
	...
	public ServerUI() {
		...
		public synchronized void send() {
			...
			textArea.setText(textArea.getText() +"[系统通知] " + txt.getText() + "\r\n");
			txt.setText(null);
		}
		...
		public void start() {
			...
			try {
				...
				serverStart(port);
				textArea.setText(textArea.getText() +"服务器已启动！端口号：" + port + "\r\n");
		...
		public void stop() {
			...
			try {
				...
				textArea.setText(textArea.getText() +"服务器已成功停止！\r\n");
			...
			public void run() {
				...
				while (true) {
					try {
						...
						textArea.setText(textArea.getText() +"[系统通知] " + clients.getUser().getName() + "上线了！\r\n");
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
	...
	class imageDownLoadThread extends Thread
	{
		...
		{
			try {
				ServerSocket imagedownloadsocket = new ServerSocket(8000);

				Socket imagesocket = imagedownloadsocket.accept();
				DataInputStream is = new DataInputStream(imagesocket.getInputStream());
				...
				while (true) {
					try {
						...
						if (message.equals("CLOSE")) { // 下线命令
							textArea.setText(textArea.getText() +"[系统通知] " + this.getUser().getName() + "下线!\r\n");
							// 断开连接的资源
							read.close();
							write.close();
							socket.close();
		...
		public synchronized void sendMessage(String message) {
			...
			else if(owner.equals("Uploadimage"))
			{
				message = name + "上传了图片文件：" + contant;
				//执行线程
			...
			else if(owner.equals("Downloadimage"))
			{
				if(file == null) {
					message = "还没有人上传过文件";
					for(int i = client.size() - 1; i >= 0; i--) {
			...
			else if(owner.equals("Finishimage"))
			{
				message = name + "下载完成";
				textArea.setText(textArea.getText() +message + "\r\n");
				for(int i = client.size() - 1; i >= 0; i--) {
